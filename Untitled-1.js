...
// ===============================
// VERIFICACIÓN DE FIREBASE (CORRECCIÓN MÍNIMA)
// ===============================
let db, auth;
try {
  if (!window.firebaseApp) {
    throw new Error('❌ Firebase no inicializado. Asegúrate de incluir la configuración en tu HTML antes de este script.');
  }
  db = window.firebaseDatabase || getDatabase(window.firebaseApp);
  auth = getAuth(window.firebaseApp);
} catch (error) {
  console.error(error.message);
  // Continuar sin Firebase para que el resto del código no falle
}
...
// Memoria local para detectar transición 0 -> >0
const prevStockById = {};
...
async function cargarProductosDesdeFirebase() {
  if (!db) {
    elementos.galeriaProductos.innerHTML = '<p class="error-carga">Error: Firebase no configurado. Revisa la consola.</p>';
    return;
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  // Verificar Firebase antes de intentar autenticar
  if (!auth) {
    mostrarNotificacion('Error crítico: Firebase no inicializado', 'error');
    return;
  }
  ...
  try {
    ...
    if (error.code === 'auth/configuration-not-found') {
      msg = '❌ Auth anónima deshabilitada. Ve a Firebase Console > Authentication > Sign-in method > Anonymous';
    } else if (error.code === 'auth/network-request-failed') {
      msg = 'Error de red. Verifica tu conexión.';
    } else if (!window.firebaseApp) {
      msg = 'Firebase no inicializado. Revisa el script en tu HTML.';
    }
