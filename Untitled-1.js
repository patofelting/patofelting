  ...
  if (elementos.galeriaProductos) {
    elementos.galeriaProductos.addEventListener('click', async (e) => {
      ...
      if (e.target.classList.contains('boton-agregar') || e.target.closest('.boton-agregar')) {
        ...
        // Click anywhere else on card opens modal
        abrirModal(producto);
      }
    });
  }
...
async function modificarCantidadCarrito(id, nuevaCantidad) {
  ...
  try {
    if (diferencia > 0) {
      // Increasing quantity - check stock and reduce
...
async function cargarProductosDesdeFirebase() {
  try {
    ...
    if (snapshot.exists()) {
      ...
      console.log('⚠️ No hay productos en Firebase, cargando desde CSV...');
...
async function cargarProductosDesdeCSV() {
  try {
    ...
    console.log(`✅ ${productos.length} productos cargados desde CSV`);
    renderizarProductos();
    actualizarCategorias();
  } catch (error) {
